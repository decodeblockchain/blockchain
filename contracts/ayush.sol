...
contract CleanLottery {
    ...
    function entry() external payable {
        ...
        require(participant[msg.sender].roundEntered != round, "Already entered this round");
    ...
    function pickWinner() external {
        uint randomIndex = uint(uint(block.prevrandao)) % maxPlayers;    // generate one random index number and we assume that index is winner 
